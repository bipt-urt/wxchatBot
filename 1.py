...
def main():
	...
	wxInitData = json.loads(bot.wxInit(wxToken))
	...
	wxContacts = json.loads(bot.getContact(wxToken))
	...
	while task:
		if task == '1':
			...
			bot.sendMessage(wxToken, contactToId, wxMessage)

			
		if task == '2':
			print("正在尝试导出联系人列表")
			bot.exportContect(wxContacts)
		...
		if task == '3':
			...
			bot.exportECharts(locationStatic)
			#print(countGroup())
